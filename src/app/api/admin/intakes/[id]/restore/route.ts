...
export async function POST(
  ...
  try {
    ...
    // Only ADMIN and EDITOR roles can restore soft-deleted intakes
    ...
    const restoredIntake = await prisma.intake.update({
      where: { id },
    ...
    return NextResponse.json({
      success: true,
      message: "Intake restored successfully",
      intake: restoredIntake,
    });
  } catch (error) {
    console.error("[Admin Intakes Restore POST Error]:", error);
    return NextResponse.json(
